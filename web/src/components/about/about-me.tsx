  ...
  return (
    <span className="group/d relative inline-block">
      ...
      <div className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 pb-2 opacity-0 scale-95 origin-bottom transition-all duration-200 ease-out group-hover/d:pointer-events-auto group-hover/d:opacity-100 group-hover/d:scale-100">
        <div className="relative rounded-lg border border-border/20 bg-[#111119] p-1.5 shadow-[0_8px_30px_rgba(0,0,0,0.5)]">
          {SOCIALS.map(({ label, href, icon: Icon }) => (
            <a
              ...
              className="flex items-center gap-2.5 rounded-md px-3 py-1.5 text-xs text-subtle transition-colors duration-100 hover:bg-white/4 hover:text-fg"
            >
              <Icon size={13} stroke={1.5} className="shrink-0 opacity-60" />
              {label}
            </a>
          ))}
          <div className="absolute -bottom-[5px] left-1/2 -translate-x-1/2 size-2.5 rotate-45 border-r border-b border-border/20 bg-[#111119]" />
        </div>
      </div>
    </span>
  )
